...
def test_load_user_function_success():
    ...
    script.write(b"def test_function(data): return 2 * data + 1\n")
    script.close()
    ...
    assert np.array_equal(loaded_function(np.array([1, 2])), np.array([3, 5]))
...
def test_apply_transformation_basic():
    ...
    script = tempfile.NamedTemporaryFile(delete=False, suffix=".py")
    script.write(b"def transform(ids, data): return data + ids\n")
    script.close()
